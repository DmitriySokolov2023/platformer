...
import Config (GenConfig (..), difficultyLevel, objectChance)
...
genFromTemplate :: GenConfig -> Int -> TemplateSpec -> ([Interval], [Rect], [Rect], [Rect])
genFromTemplate cfg ix spec =
  ...
  where
    ...
    bridgeChance = clamp01 (0.88 - fromIntegral lvl * 0.05)
