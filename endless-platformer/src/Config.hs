...
instance FromJSON ObjectRule where
  parseJSON = withObject "ObjectRule" $ \o ->
    ObjectRule <$> o .: "enabled" <*> o .: "minDifficulty" <*> o .: "baseRate"
      <*> o .: "growth"
      ...
      case eBs of
        ...
        Right bs ->
          case eitherDecode' bs of
            Left parseErr ->
              pure (Left ("Config error: cannot parse JSON: " ++ path ++ "\n" ++ parseErr))
            Right cfg ->
...
objectChance rule lvl =
  if not (objEnabled rule) || lvl < objMinDifficulty rule
    then 0
    else
      clamp01
        ( objBaseRate rule
            + objGrowth rule * fromIntegral (lvl - objMinDifficulty rule)
        )
